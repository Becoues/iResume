...
- **LLM**: OpenAI SDK against CometAPI (`https://api.cometapi.com/v1`), an aggregator that serves every model over the OpenAI Chat Completions protocol — one key, one base URL, no per-vendor protocol branching. Provider map lives in `PROVIDER_BASE_URLS` in `openai.ts`. Models: gemini-3.1-pro-preview, claude-sonnet-4-6, gpt-5.4, gemini-3.1-flash-lite-preview, qwen3.5-27b, deepseek-v3.2.
...
### Core Files
- `src/lib/openai.ts` — OpenAI SDK client: `completion()`, `streamCompletion()` (AsyncGenerator), `testConnection()`. Base URL via `PROVIDER_BASE_URLS`. Config priority: DB Settings → env vars (`COMETAPI_API_KEY`).
...
- **Settings**: singleton (id=1), provider, apiKeyCometapi, model
...
### UI Patterns
- Settings dialog: gear icon on homepage (fixed bottom-right). Provider (fixed to CometAPI) + API key + model selection + connection test.
...
JD key points, candidate profile, four-layer architecture scoring (4×5=20 max), DNA fitness (6×5=30 max), capability matrix, claims audit, project deep analysis, assessment framework, 15 technical questions (5 basic + 5 intermediate + 5 expert), 6 algorithm questions (2 easy + 2 medium + 2 hard), score card (0-100).
